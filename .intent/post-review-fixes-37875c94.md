...
- **FR-14**: `Context7Client.processBuffer` MUST cap inbound line size at `MAX_FRAME_BYTES` (10MB). A server that streams more than the cap without emitting a newline indicates either a hostile or broken MCP implementation; the buffer would otherwise grow until OOM. On overflow the process MUST be torn down via `stop()` so the next call forces a clean restart.
- **FR-17**: `Context7Client` MUST use the actual MCP stdio framing — newline-delimited JSON (`{...}\n`) — NOT LSP-style `Content-Length` framing. The earlier Content-Length implementation appeared to work in unit tests because the mocks parroted the same (incorrect) protocol; against the real `@upstash/context7-mcp` server it never received a response to `initialize` and timed out at 60s.
- **FR-18**: `Context7Client.resolveLibrary` MUST accept a `query` parameter alongside the library name, and forward both as `{ libraryName, query }` to the `resolve-library-id` MCP tool. The Context7 server's tool schema requires both. `Context7Client.queryDocs` MUST call the `query-docs` tool (renamed from the older `get-library-docs`) with `{ libraryId, query }`. Both methods MUST throw when the response carries `isError: true` rather than treating the error message as data.
...
| `src/web/context7.ts` | (#13) Identity-guard the `data` / `error` / `exit` handlers spawned in `_start`. (#14) Cap accepted line length at `MAX_FRAME_BYTES` (10MB); call `stop()` when an unframed buffer exceeds the cap. (#17) Use newline-delimited JSON framing rather than LSP-style `Content-Length` (the actual MCP stdio protocol). (#18) Adapt `resolveLibrary` and `queryDocs` to the current Context7 tool surface: `resolve-library-id` requires `{ libraryName, query }`, `query-docs` (renamed from `get-library-docs`) takes `{ libraryId, query }`; both methods throw on `isError: true`. |
...
| `test/web/context7.test.ts` | Add stale-handler regression: stop()+start() then deliver a late `exit` from the killed process; assert the new process's pending request still resolves. Add no-newline-overflow regression: 11 MB streamed without a newline triggers `stop()`. Update `frameMessage` and outbound-message parsing to newline-delimited JSON; update `resolveLibrary` callers to pass the new `query` argument. |
